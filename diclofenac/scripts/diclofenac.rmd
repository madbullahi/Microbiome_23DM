...
prevalenceThreshold <- 0.001 * nsamples(p_di)
...
dist_dic <- phyloseq::distance(
phy_di_rar, method = "bray")
ordination <- ordinate(phy_di_rar, method = "PCoA",
                       distance = dist_dic)
b.div.bray_dic <-plot_ordination(phy_di_rar, ordination,type = "samples", shape = "Treatment", color = "Genotype", title = "PCoA; taxa(Genus)")+
...
```{r}
print(b.div.bray_dic)
```

```{r}
#Phylogenetic beta diversity metrics.

dic_rel <- microbiome::transform(phy_di, "compositional") # first transform the phyloseq object
```


```{r}
dic.ord.wuni <- ordinate(dic_rel, "PCoA", "unifrac", weighted = T)
```

```{r}
b.div.wuni <- plot_ordination(dic_rel, dic.ord.wuni, type = "samples", color = "Genotype", title = "PCoA; taxa(Genus)")+
  geom_point( size = 3.5)
```
